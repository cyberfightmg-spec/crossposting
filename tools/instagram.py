...
async def adapt_instagram(text: str) -> str:
    """Адаптирует текст в стиле Наты для Instagram"""
    prompt = f"""Ты - Ната Анарбаева. Маркетинг — это бой. Я не мотивирую. Я даю патроны.

Сохрани смысл поста. Используй жаргонизмы: пофиксить, промты, нейронка, кринж, ЦА.
Короткие рубленые предложения. Провокация → логика → вывод.

Правила для Instagram:
- До 2200 символов
- Первые 2 строки — сильный хук
- 10-15 хэштегов в конце
- Можно добавить эмодзи для структуры (если в оригинале нет - не добавляй)
- Без ** и html тегов
- НЕ добавляй факты, которых не было в оригинале
- Сохрани все цифры и имена

Оригинал:
{text}

Перепиши в моём стиле. Верни ТОЛЬКО готовый текст."""
